...
describe('jsVariablePolyfill', () => {
	describe('free reads resolve through the data context', () => {
		it.each([
			['object spread', '{{ ({...value}).total }}'],
			['nested object spread', '{{ ({...({...value})}).total }}'],
			['computed object key', '{{ ({[key]: 1}).total }}'],
			['computed class field', '{{ (() => { class X { [key] = 1; } return new X().total; })() }}'],
			[
				'computed class method',
				'{{ (() => { class X { [key]() { return 1; } } return new X().total(); })() }}',
			],
			['switch case', '{{ (() => { switch (1) { case one: return 1; } })() }}'],
		])('%s', (_, expression) => {
			expect(evaluator.execute(expression, { value: { total: 1 }, key: 'total', one: 1 })).toBe(1);
		});

		it('array spread', () => {
			expect(evaluator.execute('{{ [...value].length }}', { value: ['mug', 'tee', 'cap'] })).toBe(
				3,
			);
		});

		it('call argument spread', () => {
			expect(evaluator.execute('{{ Math.max(...value) }}', { value: [12.5, 24.5, 9] })).toBe(24.5);
		});
	...
	describe('block-scoped declarations do not leak into sibling or enclosing scopes', () => {
		it.each([
			...
			expect(evaluator.execute(expression, { value: 'ORD-2048' })).toBe('ORD-2048');
		});
		...
		it('a name that is also a host global resolves through the data context', () => {
			...
			expect(evaluator.execute(expression, { process: 'shop' })).toBe('string');
		...
		it('class declaration in a sibling block does not cover a read outside it', () => {
			...
			expect(evaluator.execute(expression, { Pick: 'shop' })).toBe('shop');
		});
	});
	...
	describe('a function declared in a block is not visible outside that block', () => {
		...
		it.each([
			...
			expect(evaluator.execute(expression, { Function: 'shop', process: 'shop' })).toBe('string');
			expect(evaluator.execute(expression, {})).toBe('undefined');
		});
	...
	describe('bindings are left alone', () => {
		it.each([
			['rest property', '{{ (() => { const {...rest} = value; return rest.total; })() }}'],
			['rest element', '{{ (() => { const [...rest] = [1]; return rest[0]; })() }}'],
			['rest parameter', '{{ ((...rest) => rest[0])(1) }}'],
			[
				'local shadowing a host global',
				'{{ (() => { const process = value; return {...process}.total; })() }}',
			],
			['parameter shadowing a host global', '{{ ((process) => ({...process}).total)(value) }}'],
			[
				'declared function parameter shadowing a host global',
				'{{ (() => { function f(process) { return ({...process}).total; } return f(value); })() }}',
			],
			[
				'var in a nested block read outside the block',
				'{{ (() => { { var process = value; } return ({...process}).total; })() }}',
			],
			[
				'function declaration at function-body level shadowing a host global',
				'{{ (() => { function process() { return value; } return process().total; })() }}',
			],
			[
				'function declaration at function-body level, called before it is declared',
				'{{ (() => { return process().total; function process() { return value; } })() }}',
			],
			[
			...
			[
				...
				'{{ (() => { class process { total = value.total; } return new process().total; })() }}',
			],
			[
				'multiple declarators at function-body level shadowing a host global',
				'{{ (() => { let process = value, other = 0; return {...process}.total; })() }}',
			],
		])('%s', (_, expression) => {
			expect(evaluator.execute(expression, { value: { total: 1 } })).toBe(1);
		});
	});
});
